(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(i){if(i.ep)return;i.ep=!0;const n=s(i);fetch(i.href,n)}})();const bt="modulepreload",xt=function(e){return"/"+e},ke={},J=function(t,s,a){let i=Promise.resolve();if(s&&s.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),r=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));i=Promise.allSettled(s.map(c=>{if(c=xt(c),c in ke)return;ke[c]=!0;const d=c.endsWith(".css"),g=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${g}`))return;const m=document.createElement("link");if(m.rel=d?"stylesheet":bt,d||(m.as="script"),m.crossOrigin="",m.href=c,r&&m.setAttribute("nonce",r),document.head.appendChild(m),d)return new Promise((x,A)=>{m.addEventListener("load",x),m.addEventListener("error",()=>A(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(o){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=o,window.dispatchEvent(r),!r.defaultPrevented)throw o}return i.then(o=>{for(const r of o||[])r.status==="rejected"&&n(r.reason);return t().catch(n)})},wt={apiKey:"AIzaSyD2SUmKinx3qRGW5yehcpOpyw2sLQbmwSA",authDomain:"diggy-9eda8.firebaseapp.com",projectId:"diggy-9eda8",storageBucket:"diggy-9eda8.firebasestorage.app",messagingSenderId:"90359833058",appId:"1:90359833058:web:712187c9d78ccb2755d9bb",measurementId:"G-ZW5KTPNQ3G"};let ee=null,C=null,v=null,I=!1,w=!1,U=null,u=null;const Ge=[];let T=null;const oe=e=>`${e.toLowerCase().trim()}@diggy.com`;async function Et(){try{const e=await J(()=>import("https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js"),[]);U=await J(()=>import("https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js"),[]),u=await J(()=>import("https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js"),[]),ee=e.initializeApp(wt),C=U.getAuth(ee),v=u.getFirestore(ee),I=!0,U.onAuthStateChanged(C,async t=>{if(t)try{const s=u.doc(v,"users",t.uid),a=await u.getDoc(s);if(a.exists()){const i=a.data();D(i);return}}catch(s){console.warn("Auth state loaded but profile query failed. Falling back to local storage session.",s)}Le()}),console.log("Firebase dynamically initialized successfully.")}catch(e){console.warn("Firebase SDK failed to load from CDN. Operating in local-only fallback mode.",e),w=!0,Le()}}Et();function Le(){const e=localStorage.getItem("diggy_logged_in_uid");if(e){const s=E("users").find(a=>a.uid===e);if(s){T=s,D(s);return}}T=null,D(null)}function D(e){Ge.forEach(t=>{try{t(e)}catch(s){console.error("Error in auth listener callback:",s)}})}const K=new Map,It=5,se=15*60*1e3;function Be(e){const t=Date.now(),a=(K.get(e)||[]).filter(i=>t-i<se);return a.length>=It?{allowed:!1,remainingTime:Math.ceil((a[0]+se-t)/1e3/60)}:{allowed:!0,attempts:a.length}}function De(e){const t=Date.now(),s=K.get(e)||[];s.push(t);const a=s.filter(i=>t-i<se);K.set(e,a)}function Re(e){K.delete(e)}function O(e){return typeof e!="string"?e:e.replace(/[<>]/g,"").trim().substring(0,500)}function me(e){const t=[];return e.length<6&&t.push("הסיסמה חייבת להכיל לפחות 6 תווים"),e.length>12&&t.push("הסיסמה חייבת להכיל לכל היותר 12 תווים"),/[a-zA-Z]/.test(e)||t.push("הסיסמה חייבת להכיל לפחות אות אחת באנגלית"),/[0-9]/.test(e)||t.push("הסיסמה חייבת להכיל לפחות ספרה אחת"),{valid:t.length===0,errors:t}}function ge(e){const t=[];return e.length<6&&t.push("שם המשתמש חייב להכיל לפחות 6 תווים"),e.length>12&&t.push("שם המשתמש חייב להכיל לכל היותר 12 תווים"),/^[a-zA-Z0-9_]+$/.test(e)||t.push("שם המשתמש יכול להכיל רק אותיות, ספרות וקו תחתון"),{valid:t.length===0,errors:t}}function V(e){return{valid:/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e),error:"כתובת האימייל אינה תקינה"}}function W(e){return e==="developer"||e==="admin"}function fe(e){if(!e||!W(e.role))return{required:!1,complete:!0,missingItems:[]};const t=[];return e.twoFactorEnabled||t.push("twoFactor"),(!e.supportEmail||!V(e.supportEmail).valid)&&t.push("supportEmail"),{required:!0,complete:t.length===0,missingItems:t}}function E(e){const t=localStorage.getItem(`diggy_db_${e}`);return t?JSON.parse(t):[]}function L(e,t){localStorage.setItem(`diggy_db_${e}`,JSON.stringify(t))}E("games").length===0&&L("games",[{id:"preset_snake",name:"Neon Snake",description:"The classic retro arcade game! Guide the neon snake to consume glowing particles, but avoid hitting yourself or the boundaries.",logoUrl:"https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=150&auto=format&fit=crop&q=60",githubUrl:"https://github.com/diggy-games/neon-snake",howToPlay:"Use the arrow keys or WASD to navigate the snake. Eat green glowing particles to grow. The game ends if you collide with the walls or your own tail.",targetAudience:"Everyone (All Ages)",categories:["RETRO","RPG"],developerUid:"system",developerName:"DIGGY Core Devs",approved:!0}]);if(E("users").length===0){const e=[{uid:"local_admin_123",username:"admin",email:"admin@diggy.com",role:"admin",twoFactorEnabled:!1,twoFactorEmail:"",supportEmail:"",biometricsEnabled:!1,customTheme:"#00ff66",favorites:[],recentlyPlayed:[],createdAt:new Date().toISOString()},{uid:"local_dev_456",username:"developer_jon",email:"jon@diggy.com",role:"developer",twoFactorEnabled:!1,twoFactorEmail:"",supportEmail:"",biometricsEnabled:!1,customTheme:"#00ffff",favorites:["preset_snake"],recentlyPlayed:[],createdAt:new Date().toISOString()},{uid:"local_player_789",username:"gamer_kid",email:"kid@diggy.com",role:"player",twoFactorEnabled:!1,twoFactorEmail:"",supportEmail:"",biometricsEnabled:!1,customTheme:"#ff3366",favorites:[],recentlyPlayed:["preset_snake"],createdAt:new Date().toISOString()}];L("users",e)}function Ue(e){Ge.push(e),(w||I)&&e(T)}async function Pe(e,t){const s=e.trim();if(s.length<6||s.length>12)throw new Error("Username must be between 6 and 12 characters.");if(t.length<6||t.length>12)throw new Error("Password must be between 6 and 12 characters.");const i={uid:"local_"+Math.random().toString(36).substr(2,9),username:s,email:oe(s),role:s.toLowerCase()==="admin"?"admin":"player",twoFactorEnabled:!1,twoFactorEmail:"",supportEmail:"",biometricsEnabled:!1,biometricsCredential:null,customTheme:"#00ff66",favorites:[],recentlyPlayed:[],createdAt:new Date().toISOString()};if(I&&!w)try{const r=oe(s),c=u.collection(v,"users"),d=u.query(c,u.where("username","==",s));if(!(await u.getDocs(d)).empty)throw new Error("Username is already taken.");const x=(await U.createUserWithEmailAndPassword(C,r,t)).user;i.uid=x.uid,await u.setDoc(u.doc(v,"users",x.uid),i);const A=E("users");return A.push(i),L("users",A),T=i,localStorage.setItem("diggy_logged_in_uid",x.uid),D(i),i}catch(r){if(console.warn("Firebase sign up failed. Falling back to LocalStorage auth.",r),r.code==="auth/email-already-in-use"||r.message==="Username is already taken.")throw new Error("שם המשתמש כבר תפוס במערכת!");if(r.code==="auth/weak-password")throw new Error("הסיסמה חלשה מדי!");w=!0,console.log("Switched to LocalStorage fallback due to error:",r.message||r)}const n=E("users");if(n.some(r=>r.username.toLowerCase()===s.toLowerCase()))throw new Error("Username is already taken.");return n.push(i),L("users",n),T=i,localStorage.setItem("diggy_logged_in_uid",i.uid),D(i),i}async function ye(e,t){const s=e.trim().toLowerCase();if(I&&!w&&t!=="DUMMY_PASSWORD_NOT_USED"&&t!=="auth_biometric_token")try{const n=oe(e),r=(await U.signInWithEmailAndPassword(C,n,t)).user,c=u.doc(v,"users",r.uid),d=await u.getDoc(c);if(d.exists()){const g=d.data();return T=g,localStorage.setItem("diggy_logged_in_uid",g.uid),D(g),g}throw new Error("User profile not found in database.")}catch(n){if(console.warn("Firebase sign in failed. Attempting LocalStorage auth fallback.",n),n.code==="auth/user-not-found"||n.code==="auth/wrong-password"||n.code==="auth/invalid-credential"||n.code==="auth/invalid-email")throw new Error("שם המשתמש או הסיסמה שגויים!");w=!0,console.log("Switched to LocalStorage login fallback due to error:",n.message||n)}const i=E("users").find(n=>n.username.toLowerCase()===s);if(!i)throw new Error("שם המשתמש או הסיסמה שגויים! (לא נמצא חשבון)");return T=i,localStorage.setItem("diggy_logged_in_uid",i.uid),D(i),i}async function Ce(){if(localStorage.removeItem("diggy_logged_in_uid"),T=null,I&&!w)try{await U.signOut(C)}catch(e){console.warn("Firebase sign out failed:",e)}D(null)}async function Me(e){if(I&&!w)try{const a=u.doc(v,"users",e),i=await u.getDoc(a);if(i.exists())return i.data()}catch(a){console.warn("Firebase profile read failed, using local fallback:",a)}const s=E("users").find(a=>a.uid===e);if(s)return s;throw new Error("User profile not found.")}async function _(e,t){const s=E("users"),a=s.findIndex(i=>i.uid===e);if(a!==-1&&(s[a]={...s[a],...t},L("users",s),T&&T.uid===e&&(T=s[a])),I&&!w)try{const i=u.doc(v,"users",e);await u.updateDoc(i,t);return}catch(i){console.warn("Firebase profile update failed, saved locally only:",i)}}async function qe(e){if(e.length<6||e.length>12)throw new Error("Password must be between 6 and 12 characters.");if(I&&!w)try{const t=C.currentUser;if(t){await U.updatePassword(t,e);return}}catch(t){console.warn("Firebase password change failed, falling back to local only:",t)}console.log("Local password updated successfully.")}async function ze(){if(I&&!w)try{const e=u.query(u.collection(v,"users")),t=await u.getDocs(e),s=[];return t.forEach(a=>s.push(a.data())),s}catch(e){console.warn("Firebase load all users failed, loading local:",e)}return E("users")}async function Fe(e,t){await _(e,{role:t})}async function Oe(e,t,s,a){const i={id:"req_"+Math.random().toString(36).substr(2,9),uid:e,username:t,reason:s,contactEmail:a,status:"pending",createdAt:new Date().toISOString(),adminReason:""},n=E("developer_requests");if(n.some(r=>r.uid===e&&r.status==="pending"))throw new Error("יש לך כבר פנייה ממתינה להפוך למפתח!");if(n.push(i),L("developer_requests",n),I&&!w)try{await u.addDoc(u.collection(v,"developer_requests"),i)}catch(r){console.warn("Firebase dev request submission failed, saved locally only:",r)}return i}async function je(){if(I&&!w)try{const e=u.query(u.collection(v,"developer_requests"),u.orderBy("createdAt","desc")),t=await u.getDocs(e),s=[];return t.forEach(a=>s.push({id:a.id,...a.data()})),s}catch(e){console.warn("Firebase developer requests load failed, loading local:",e)}return E("developer_requests").sort((e,t)=>new Date(t.createdAt)-new Date(e.createdAt))}async function He(e,t,s){console.log("handleDeveloperRequest called with:",{requestId:e,status:t,adminReason:s});const a=E("developer_requests");console.log("Current requests:",a);const i=a.findIndex(o=>o.id===e||o.uid===e);console.log("Found request at index:",i);let n=null;if(i!==-1)a[i].status=t,a[i].adminReason=s,n=a[i],L("developer_requests",a),t==="approved"&&await _(a[i].uid,{role:"developer"});else throw console.error("Request not found with id:",e),new Error("Request not found - could not locate developer request with ID: "+e);if(I&&!w&&n)try{const o=u.collection(v,"developer_requests"),r=u.query(o,u.where("uid","==",n.uid)),c=await u.getDocs(r);if(!c.empty){const d=c.docs[0].id;await u.updateDoc(u.doc(v,"developer_requests",d),{status:t,adminReason:s})}}catch(o){console.warn("Firebase developer request handle failed, processed locally:",o)}if(n)return await ot(n.contactEmail,n.username,"Developer Role Application",t,s),n;throw new Error("Request not found")}async function Ye(e){const s={id:"greq_"+Math.random().toString(36).substr(2,9),...e,status:"pending",createdAt:new Date().toISOString(),adminSuggestions:""},a=E("game_requests");if(a.some(n=>n.githubUrl===e.githubUrl&&n.status==="rejected"))throw new Error("מאגר המשחק הזה נדחה בעבר ולא ניתן להגישו שוב.");if(a.push(s),L("game_requests",a),I&&!w)try{await u.addDoc(u.collection(v,"game_requests"),s)}catch(n){console.warn("Firebase game request failed, saved locally:",n)}return s}async function Ne(e){if(I&&!w)try{const t=u.query(u.collection(v,"game_requests"),u.where("developerUid","==",e)),s=await u.getDocs(t),a=[];return s.forEach(i=>a.push({id:i.id,...i.data()})),a}catch(t){console.warn("Firebase load dev game requests failed, loading local:",t)}return E("game_requests").filter(t=>t.developerUid===e)}async function Je(){if(I&&!w)try{const e=u.query(u.collection(v,"game_requests")),t=await u.getDocs(e),s=[];return t.forEach(a=>s.push({id:a.id,...a.data()})),s}catch(e){console.warn("Firebase load pending game requests failed, loading local:",e)}return E("game_requests")}async function Ke(e,t,s=""){const a=E("game_requests"),i=a.findIndex(o=>o.id===e);let n=null;if(i!==-1&&(a[i].status=t,a[i].adminSuggestions=s,n=a[i],L("game_requests",a),t==="approved")){const o=E("games");if(n.type==="version_update"){const r=o.findIndex(c=>c.id===n.parentGameId);r!==-1&&(o[r].gameUrl=n.gameUrl,o[r].githubUrl=n.githubUrl,o[r].version=n.version,o[r].latestChangelog=n.changelog,L("games",o))}else{const r="game_"+Math.random().toString(36).substr(2,9),c={id:r,name:n.name,description:n.description,logoUrl:n.logoUrl,githubUrl:n.githubUrl,gameUrl:n.gameUrl||"",howToPlay:n.howToPlay,targetAudience:n.targetAudience,categories:n.categories,developerUid:n.developerUid,developerName:n.developerName,approved:!0,plays:0,ratingSum:0,ratingCount:0,rating:5,createdAt:new Date().toISOString()};o.push(c),L("games",o),a[i].gameId=r,L("game_requests",a)}}if(I&&!w&&n)try{const o=u.collection(v,"game_requests"),r=u.query(o,u.where("id","==",e)),c=await u.getDocs(r);if(!c.empty){const d=c.docs[0].id;let g={status:t,adminSuggestions:s};if(t==="approved")if(n.type==="version_update"){const m=u.collection(v,"games"),x=u.query(m,u.where("id","==",n.parentGameId)),A=await u.getDocs(x);if(!A.empty){const b=A.docs[0].id;await u.updateDoc(u.doc(v,"games",b),{gameUrl:n.gameUrl,githubUrl:n.githubUrl,version:n.version,latestChangelog:n.changelog})}}else{const m="game_"+Math.random().toString(36).substr(2,9);await u.addDoc(u.collection(v,"games"),{id:m,name:n.name,description:n.description,logoUrl:n.logoUrl,githubUrl:n.githubUrl,gameUrl:n.gameUrl||"",howToPlay:n.howToPlay,targetAudience:n.targetAudience,categories:n.categories,developerUid:n.developerUid,developerName:n.developerName,approved:!0,plays:0,ratingSum:0,ratingCount:0,rating:5,createdAt:new Date().toISOString()}),g.gameId=m}await u.updateDoc(u.doc(v,"game_requests",d),g)}}catch(o){console.warn("Firebase game request handling error, completed locally:",o)}if(n){try{const o=await Me(n.developerUid),r=o.supportEmail||o.twoFactorEmail||o.email||"diggy-games@outlook.com";await ot(r,n.developerName,`Game Submission: ${n.name}`,t,s)}catch(o){console.warn("Failed to send notification email:",o)}return n}throw new Error("Request not found")}async function Ve(e,t){const s=E("game_requests"),a=s.findIndex(i=>i.id===e);if(a!==-1&&(s[a]={...s[a],...t,status:"pending",adminSuggestions:"",createdAt:new Date().toISOString()},L("game_requests",s)),I&&!w)try{const i=u.collection(v,"game_requests"),n=u.query(i,u.where("githubUrl","==",t.githubUrl)),o=await u.getDocs(n);if(!o.empty){const r=o.docs[0].id;await u.updateDoc(u.doc(v,"game_requests",r),{...t,status:"pending",adminSuggestions:"",createdAt:new Date().toISOString()})}}catch(i){console.warn("Firebase resubmission failed, updated locally:",i)}}async function We(e){const t={id:"game_"+Math.random().toString(36).substr(2,9),...e,approved:!0,createdAt:new Date().toISOString()},s=E("games");if(s.push(t),L("games",s),I&&!w)try{await u.addDoc(u.collection(v,"games"),t)}catch(a){console.warn("Firebase direct publish failed, published locally:",a)}return t}async function Ze(){if(I&&!w)try{const e=u.query(u.collection(v,"games"),u.orderBy("createdAt","desc")),t=await u.getDocs(e),s=[];return t.forEach(a=>s.push({id:a.id,...a.data()})),s}catch(e){console.warn("Firebase load active games failed, loading local:",e)}return E("games")}const M=new Map;function ne(e){const t=Math.floor(1e5+Math.random()*9e5).toString(),s=Date.now()+5*60*1e3;return M.set(e,{code:t,expiresAt:s,attempts:0,maxAttempts:3}),t}function Xe(e,t){const s=M.get(e);return s?Date.now()>s.expiresAt?(M.delete(e),{valid:!1,error:"קוד האימות פג תוקף. בקש קוד חדש."}):s.attempts>=s.maxAttempts?(M.delete(e),{valid:!1,error:"חרגת ממספר הניסיונות המקסימלי. נסה להתחבר מחדש."}):(s.attempts++,t===s.code?(M.delete(e),{valid:!0}):{valid:!1,error:`קוד שגוי. נותרו ${s.maxAttempts-s.attempts} ניסיונות.`}):{valid:!1,error:"קוד אימות לא תקף או פג תוקף"}}function Qe(e){M.delete(e)}const j=new Map;async function et(e,t){if(!window.PublicKeyCredential)throw new Error("הדפדפן שלך לא תומך ב-WebAuthn");if(!await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable())throw new Error("לא נמצא מכשיר ביומטרי זמין במערכת");try{const a=new TextEncoder().encode(t),i=new Uint8Array(32);crypto.getRandomValues(i);const n={challenge:i,rp:{name:"DIGGY Games",id:window.location.hostname||"localhost"},user:{id:a,name:e,displayName:e},pubKeyCredParams:[{type:"public-key",alg:-7}],authenticatorSelection:{authenticatorAttachment:"platform",userVerification:"required"},timeout:6e4},o=await navigator.credentials.create({publicKey:n});if(!o)throw new Error("יצירת אישור ביומטרי נכשלה");const r=Array.from(new Uint8Array(o.rawId)),c={credentialId:r,publicKey:o.response.publicKey?Array.from(new Uint8Array(o.response.publicKey)):null,counter:0,username:e,uid:t,createdAt:Date.now()};return j.set(t,c),localStorage.setItem(`diggy_webauthn_${t}`,JSON.stringify(c)),{success:!0,credentialId:r}}catch(a){throw console.error("WebAuthn registration error:",a),new Error(`שגיאה ברישום ביומטרי: ${a.message}`)}}async function tt(e,t){if(!window.PublicKeyCredential)throw new Error("הדפדפן שלך לא תומך ב-WebAuthn");let s=j.get(t);if(!s){const a=localStorage.getItem(`diggy_webauthn_${t}`);a&&(s=JSON.parse(a),j.set(t,s))}if(!s)throw new Error("לא נמצא אישור ביומטרי שמור. אנא הפעל זיהוי ביומטרי בהגדרות.");try{const a=new Uint8Array(32);crypto.getRandomValues(a);const i={challenge:a,allowCredentials:[{type:"public-key",id:new Uint8Array(s.credentialId)}],userVerification:"required",timeout:6e4};if(!await navigator.credentials.get({publicKey:i}))throw new Error("אימות ביומטרי נכשל");return s.counter++,localStorage.setItem(`diggy_webauthn_${t}`,JSON.stringify(s)),{success:!0,username:e}}catch(a){throw console.error("WebAuthn verification error:",a),new Error(`שגיאה באימות ביומטרי: ${a.message}`)}}function St(e){return j.has(e)||localStorage.getItem(`diggy_webauthn_${e}`)!==null}function At(e){j.delete(e),localStorage.removeItem(`diggy_webauthn_${e}`)}const re=[];function X(){const e=typeof window<"u"?window:null,t=(localStorage.getItem("diggy_emailjs_service_id")||(e==null?void 0:e.__DIGGY_EMAILJS_SERVICE_ID__)||"").trim(),s=(localStorage.getItem("diggy_emailjs_template_id")||(e==null?void 0:e.__DIGGY_EMAILJS_TEMPLATE_ID__)||"").trim(),a=(localStorage.getItem("diggy_emailjs_public_key")||(e==null?void 0:e.__DIGGY_EMAILJS_PUBLIC_KEY__)||"").trim(),i=(localStorage.getItem("diggy_emailjs_from_name")||(e==null?void 0:e.__DIGGY_EMAILJS_FROM_NAME__)||"DIGGY Games").trim();return{serviceId:t,templateId:s,publicKey:a,fromName:i||"DIGGY Games",enabled:!!(t&&s&&a)}}function at(e,t,s,a){const i=(e||"").trim(),n=(t||"").trim(),o=(s||"").trim(),r=(a||"").trim();return localStorage.setItem("diggy_emailjs_service_id",i),localStorage.setItem("diggy_emailjs_template_id",n),localStorage.setItem("diggy_emailjs_public_key",o),localStorage.setItem("diggy_emailjs_from_name",r||"DIGGY Games"),X()}function it(e,t,s,a){return at(e,t,s,a)}function kt(){return X()}function F(){return X()}function Lt(e){return String(e||"").trim()||"diggy-games@outlook.com"}async function H(e,t,s){const a=Lt(e),i={id:"email_"+Math.random().toString(36).substr(2,9),to:a,subject:t,html:s,sentAt:new Date().toLocaleTimeString(),timestamp:Date.now()},n=X();if(n.enabled&&typeof window<"u"&&window.emailjs)try{n.publicKey&&typeof window.emailjs.init=="function"&&window.emailjs.init(n.publicKey);const o=await window.emailjs.send(n.serviceId,n.templateId,{to:a,to_email:a,email:a,recipient:a,subject:t,message:s,message_html:s,reply_to:a,replyTo:a,from_name:n.fromName},n.publicKey);if((o==null?void 0:o.status)===200)i.status="sent",i.messageId=(o==null?void 0:o.text)||(o==null?void 0:o.id),console.log(`[Email Sent via EmailJS] to: ${a} | subject: ${t} | id: ${(o==null?void 0:o.text)||(o==null?void 0:o.id)}`);else throw new Error("EmailJS failed to send the message.")}catch(o){return console.error("[EmailJS Error]",o),i.status="failed",i.error=o.message,re.unshift(i),window.dispatchEvent(new CustomEvent("diggy-email-sent",{detail:i})),{success:!1,mode:"emailjs_failed",error:o.message,email:i}}else i.status="simulated",console.log(`[Email Simulated] to: ${a} | subject: ${t}`);return re.unshift(i),window.dispatchEvent(new CustomEvent("diggy-email-sent",{detail:i})),{success:!0,mode:n.enabled?"emailjs":"simulated",email:i}}async function ot(e,t,s,a,i){const n={approved:"#00ff66",rejected:"#ff3366",improvement:"#ffcc00"},o={approved:"APPROVED / מאושר",rejected:"REJECTED / נדחה",improvement:"IMPROVEMENTS REQUESTED / דרוש שיפור"},r=n[a]||"#00ff66",c=o[a]||a.toUpperCase(),d=`
  ...
  `;await H(e,`DIGGY - עדכון בקשת ${s}`,d)}async function st(e){const t=E("games"),s=t.findIndex(a=>a.id===e);if(s!==-1&&(t[s].plays=(t[s].plays||0)+1,L("games",t)),I&&!w)try{const a=u.collection(v,"games"),i=u.query(a,u.where("id","==",e)),n=await u.getDocs(i);if(n.empty){const o=u.doc(v,"games",e),r=await u.getDoc(o);if(r.exists()){const c=r.data().plays||0;await u.updateDoc(o,{plays:c+1})}}else{const o=n.docs[0].id,r=n.docs[0].data().plays||0;await u.updateDoc(u.doc(v,"games",o),{plays:r+1})}}catch(a){console.warn("Firebase record gameplay failed:",a)}}async function nt(e,t){const a={id:"greq_"+Math.random().toString(36).substr(2,9),parentGameId:e,type:"version_update",status:"pending",createdAt:new Date().toISOString(),adminSuggestions:"",...t},i=E("game_requests");if(i.push(a),L("game_requests",i),I&&!w)try{await u.addDoc(u.collection(v,"game_requests"),a)}catch(n){console.warn("Firebase game version request failed, saved locally:",n)}return a}async function rt(e,t){const s=E("games"),a=s.findIndex(n=>n.id===e);let i=5;if(a!==-1){const n=s[a].ratingSum||0,o=s[a].ratingCount||0,r=n+t,c=o+1;i=parseFloat((r/c).toFixed(1)),s[a].ratingSum=r,s[a].ratingCount=c,s[a].rating=i,L("games",s)}if(I&&!w)try{const n=u.collection(v,"games"),o=u.query(n,u.where("id","==",e)),r=await u.getDocs(o);if(!r.empty){const c=r.docs[0].id,d=r.docs[0].data(),g=d.ratingSum||0,m=d.ratingCount||0,x=g+t,A=m+1,b=parseFloat((x/A).toFixed(1));await u.updateDoc(u.doc(v,"games",c),{ratingSum:x,ratingCount:A,rating:b})}}catch(n){console.warn("Firebase rate game failed:",n)}return i}const $t=Object.freeze(Object.defineProperty({__proto__:null,get auth(){return C},changeUserPassword:qe,changeUserRole:Fe,checkLoginRateLimit:Be,clear2FACode:Qe,clearLoginAttempts:Re,directPublishGame:We,generateAndStore2FACode:ne,getActiveGames:Ze,getAllUsers:ze,getDeveloperGameRequests:Ne,getDeveloperRequests:je,getEmailJSConfigState:kt,getPendingGameRequests:Je,getPrivilegedAccountRequirements:fe,getResendConfigState:F,getUserProfile:Me,handleDeveloperRequest:He,handleGameRequest:Ke,hasWebAuthnCredential:St,isPrivilegedRole:W,logInUser:ye,logOutUser:Ce,onAuthStateListener:Ue,rateGame:rt,recordGamePlay:st,recordLoginAttempt:De,registerWebAuthnCredential:et,removeWebAuthnCredential:At,sanitizeInput:O,sendEmailViaResend:H,setEmailJSConfig:at,setResendConfig:it,signUpUser:Pe,simulatedEmails:re,submitDeveloperRequest:Oe,submitGameRequest:Ye,submitGameVersionRequest:nt,updateAndResubmitGameRequest:Ve,updateUserProfile:_,validateEmail:V,validatePasswordStrength:me,validateUsername:ge,verify2FACode:Xe,verifyWebAuthnCredential:tt},Symbol.toStringTag,{value:"Module"}));let l={user:null,currentRoute:"#/",games:[],theme:"#00ff66",activePromoIndex:0,promoTimer:null,currentGame:null,gameInstance:null,recentEmails:[],supportActiveThreadId:null};const te=[{id:"preset_snake",name:"Neon Snake",description:"The classic retro arcade game! Guide the neon snake to consume glowing particles, but avoid hitting yourself or the boundaries.",logoUrl:"https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=150&auto=format&fit=crop&q=60",githubUrl:"https://github.com/diggy-games/neon-snake",howToPlay:"Use the arrow keys or WASD to navigate the snake. Eat green glowing particles to grow. The game ends if you collide with the walls or your own tail.",targetAudience:"Everyone (All Ages)",categories:["RETRO","RPG"],developerUid:"system",developerName:"DIGGY Core Devs",approved:!0,rating:4.8,ratingCount:156,ratingSum:748.8},{id:"preset_bricks",name:"Brick Breaker Glow",description:"Bounce the ball and destroy the neon bricks in this fast-paced arcade retro classic. Collect multipliers and clear the screen!",logoUrl:"https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=150&auto=format&fit=crop&q=60",githubUrl:"https://github.com/diggy-games/brick-breaker-glow",howToPlay:"Move the paddle left and right using your Mouse or Left/Right arrow keys. Prevent the glowing orb from falling. Break all the colored neon bricks to win.",targetAudience:"Kids 6+",categories:["RETRO","MULTIPLAYER"],developerUid:"system",developerName:"DIGGY Core Devs",approved:!0,rating:4.6,ratingCount:98,ratingSum:450.8},{id:"preset_evader",name:"Space Laser Evader",description:"Navigate your starfighter through an intense neon asteroid field. Shoot incoming targets and survive the onslaught!",logoUrl:"https://images.unsplash.com/photo-1542751371-adc38448a05e?w=150&auto=format&fit=crop&q=60",githubUrl:"https://github.com/diggy-games/space-laser-evader",howToPlay:"Move Left/Right using the Arrow keys or A/D keys. Fire your laser cannon using the Spacebar. Avoid colliding with space debris.",targetAudience:"Teens 10+",categories:["RPG","RETRO"],developerUid:"system",developerName:"DIGGY Core Devs",approved:!0,rating:4.9,ratingCount:203,ratingSum:994.7}];function ve(e){const t=parseFloat(e.rating??5),s=e.ratingCount??0;return{rating:t,count:s,display:t.toFixed(1)}}function lt(e,t,s=""){const a=parseFloat(e);let i="";for(let o=1;o<=5;o++)a>=o-.25?i+='<i class="fas fa-star"></i>':a>=o-.75?i+='<i class="fas fa-star-half-alt"></i>':i+='<i class="far fa-star"></i>';const n=t>0?`<span class="rating-count">(${t})</span>`:"";return`<div class="star-rating-display ${s}">${i}<span class="rating-value">${a.toFixed(1)}</span>${n}</div>`}function dt(){try{return JSON.parse(localStorage.getItem("diggy_game_ratings")||"{}")}catch{return{}}}function _t(e,t){var i;const s=dt(),a=((i=l.user)==null?void 0:i.uid)||"guest";s[a]||(s[a]={}),s[a][e]=t,localStorage.setItem("diggy_game_ratings",JSON.stringify(s))}function N(e){var a,i;const t=dt(),s=((a=l.user)==null?void 0:a.uid)||"guest";return((i=t[s])==null?void 0:i[e])||null}function he(){try{return JSON.parse(localStorage.getItem("diggy_support_threads")||"[]")}catch{return[]}}function P(){try{return{supportEmail:"diggy-games@outlook.com",legalEmail:"diggy-games@outlook.com",notificationEmail:"diggy-games@outlook.com",...JSON.parse(localStorage.getItem("diggy_email_settings")||"{}")}}catch{return{supportEmail:"diggy-games@outlook.com",legalEmail:"diggy-games@outlook.com",notificationEmail:"diggy-games@outlook.com"}}}function Tt(e){const s={...P(),...e};return localStorage.setItem("diggy_email_settings",JSON.stringify(s)),s}function ct(e){localStorage.setItem("diggy_support_threads",JSON.stringify(e))}function Gt({name:e,email:t,subject:s,message:a}){const i={id:"support_"+Math.random().toString(36).slice(2,10),name:e,email:t,subject:s,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),messages:[{id:"msg_"+Math.random().toString(36).slice(2,10),sender:"user",author:e,text:a,createdAt:new Date().toISOString()}]},n=he();return n.unshift(i),ct(n),i}function Bt(e,t,s,a){const i=he(),n=i.find(o=>o.id===e);return n?(n.messages.push({id:"msg_"+Math.random().toString(36).slice(2,10),sender:t,author:s,text:a,createdAt:new Date().toISOString()}),n.updatedAt=new Date().toISOString(),ct(i),n):null}function le(e=null){var c;const t=document.getElementById("admin-support-thread-list"),s=document.getElementById("admin-support-thread-content");if(!t||!s)return;const a=he().sort((d,g)=>new Date(g.updatedAt||g.createdAt)-new Date(d.updatedAt||d.createdAt)),i=e||l.supportActiveThreadId||((c=a[0])==null?void 0:c.id)||null;if(l.supportActiveThreadId=i,a.length===0){t.innerHTML='<div class="support-chat-empty">אין פניות פתוחות כרגע.</div>',s.innerHTML='<div class="support-chat-empty">לא נבחרה פנייה.</div>';return}t.innerHTML=a.map(d=>{const g=d.messages[d.messages.length-1];return`
    ...
    `}).join(""),t.querySelectorAll(".support-thread-card").forEach(d=>{d.addEventListener("click",()=>{le(d.getAttribute("data-thread-id"))})});const n=a.find(d=>d.id===i)||a[0],o=n.messages.map(d=>`
  ...
  `).join("");s.innerHTML=`
    ...
    <div class="support-thread-messages">${o}</div>
  ...
  `;const r=document.getElementById("support-reply-form");r&&r.addEventListener("submit",async d=>{var b;d.preventDefault();const m=document.getElementById("support-reply-input").value.trim();if(!m)return;Bt(n.id,"admin",((b=l.user)==null?void 0:b.username)||"Admin",m);const x=n.email,A=`
      ...
      `;await H(x,"DIGGY - תגובה חדשה לתמיכה",A),p("התגובה נשלחה למשתמש!","success"),le(n.id)})}function ut(e){const t=document.getElementById("game-rating-input"),s=document.getElementById("game-rating-display");if(!t)return;const a=l.games.find(r=>r.id===e);if(!a)return;const i=N(e),{rating:n,count:o}=ve(a);s&&(s.innerHTML=lt(n,o)),t.innerHTML=`
  ...
  `,t.querySelectorAll(".star-input-btn").forEach(r=>{r.addEventListener("click",async()=>{const c=parseInt(r.getAttribute("data-score"),10);if(N(e)){p("כבר דירגת משחק זה!","warning");return}y(!0);try{const d=await rt(e,c);_t(e,c);const g=l.games.findIndex(m=>m.id===e);g!==-1&&(l.games[g].rating=d,l.games[g].ratingCount=(l.games[g].ratingCount||0)+1,l.games[g].ratingSum=(l.games[g].ratingSum||0)+c),p("תודה על הדירוג! ⭐","success"),ut(e)}catch{p("שגיאה בשמירת הדירוג","danger")}finally{y(!1)}}),r.addEventListener("mouseenter",()=>{if(N(e))return;const c=parseInt(r.getAttribute("data-score"),10);t.querySelectorAll(".star-input-btn").forEach(d=>{const g=parseInt(d.getAttribute("data-score"),10),m=d.querySelector("i");m.className=g<=c?"fas fa-star":"far fa-star"})}),r.addEventListener("mouseleave",()=>{const c=N(e);t.querySelectorAll(".star-input-btn").forEach(d=>{const g=parseInt(d.getAttribute("data-score"),10),m=d.querySelector("i");m.className=c&&g<=c?"fas fa-star":"far fa-star",d.classList.toggle("selected",c&&g<=c)})})})}const Dt={"#/":pt,"#/login":Ct,"#/dev":xe,"#/dev-docs":Zt,"#/admin":Q,"#/settings":ft,"#/articles":Yt,"#/sitemap":Jt,"#/terms":Kt,"#/privacy":Vt,"#/contact":Wt,"#/game/:id":gt};function k(e){window.location.hash=e}async function $e(){const e=window.location.hash||"#/";if(l.currentRoute=e,l.gameInstance&&l.gameInstance.stop&&(l.gameInstance.stop(),l.gameInstance=null),e.startsWith("#/game/")){const s=e.split("#/game/")[1];await gt(s),ae("");return}if(e.startsWith("#/articles/")){const s=e.split("#/articles/")[1];await Nt(s),ae("#/articles");return}const t=Dt[e]||pt;ae(e),await t()}function ae(e){document.querySelectorAll(".nav-item").forEach(t=>{t.getAttribute("data-route")===e?t.classList.add("active"):t.classList.remove("active")})}window.addEventListener("DOMContentLoaded",async()=>{window.addEventListener("hashchange",$e),window.addEventListener("diggy-email-sent",()=>{l.recentEmails=l.recentEmails.slice(0,5)}),ie(),Rt(),Ue(async e=>{e?(l.user=e,pe(e.customTheme||"#00ff66"),Z(),ie()):(l.user=null,pe("#00ff66"),Z(),ie()),$e()}),await be()});function ie(){const e=document.getElementById("sidebar-nav-menu");if(!e)return;let t=`
  ...
  `,l.user&&((l.user.role==="developer"||l.user.role==="admin")&&(t+=`
    ...
    `,document.head.appendChild(e)}}async function be(){try{const e=await Ze();l.games=[...te,...e.filter(t=>!te.some(s=>s.id===t.id))]}catch(e){console.warn("Could not pull games from Firebase, using presets only:",e),l.games=[...te]}}async function pt(){const e=document.getElementById("main-container");e.innerHTML=`
  ...
  `,Ut(),Pt(),de("ALL"),mt();const t=e.querySelectorAll(".category-tabs button");t.forEach(s=>{s.addEventListener("click",()=>{t.forEach(a=>{a.classList.remove("active-cat"),a.style.borderColor="rgba(255, 255, 255, 0.05)",a.style.background="transparent"}),s.classList.add("active-cat"),s.style.borderColor="var(--accent-color)",s.style.background="var(--accent-dim)",de(s.getAttribute("data-category"))})})}function Ut(){const e=document.getElementById("header-auth-actions");e&&(l.user?(e.innerHTML=`
    ...
    `)}function Pt(){const e=document.getElementById("promo-slider");if(!e)return;const t=l.games.slice(0,3);if(t.length===0){e.style.display="none";return}clearInterval(l.promoTimer),e.innerHTML=t.map((s,a)=>`
    <div class="slide-item ${a===0?"active":""}" style="background-image: url('${s.logoUrl}')" data-index="${a}">
      <div class="slide-overlay"></div>
      <div class="slide-content">
        <span class="slide-tag">משחק מומלץ!</span>
        <h2 class="slide-title">${s.name}</h2>
        <p class="slide-desc">${s.description}</p>
        <button class="btn btn-primary play-now-promo" data-id="${s.id}"><i class="fas fa-play"></i> שחק עכשיו</button>
      </div>
    </div>
  `).join(""),e.querySelectorAll(".play-now-promo").forEach(s=>{s.addEventListener("click",()=>{const a=s.getAttribute("data-id");k(`#/game/${a}`)})}),l.activePromoIndex=0,l.promoTimer=setInterval(()=>{const s=e.querySelectorAll(".slide-item");s.length&&(s[l.activePromoIndex].classList.remove("active"),l.activePromoIndex=(l.activePromoIndex+1)%s.length,s[l.activePromoIndex].classList.add("active"))},5e3)}function de(e){const t=document.getElementById("home-games-grid");if(!t)return;console.log("renderGamesGrid called with category:",e),console.log("Total games:",l.games.length),console.log("Games with categories:",l.games.filter(a=>a.categories&&a.categories.length>0).length);const s=e==="ALL"?l.games:l.games.filter(a=>a.categories&&a.categories.includes(e));if(console.log("Filtered games count:",s.length),console.log("Filtered games:",s.map(a=>({name:a.name,categories:a.categories}))),s.length===0){t.innerHTML='<div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px 0;">אין משחקים בקטגוריה זו כרגע.</div>';return}t.innerHTML=s.map(a=>ce(a)).join(""),ue(t)}function ce(e){const t=l.user&&l.user.favorites&&l.user.favorites.includes(e.id),s=t?"active":"",a=t?"fas fa-heart":"far fa-heart",{rating:i,count:n}=ve(e);return`
    <div class="game-card" data-id="${e.id}">
      <div class="game-card-image" style="background-image: url('${e.logoUrl}')">
        ...
        <button class="favorite-btn ${s}" data-id="${e.id}">
          <i class="${a}"></i>
        </button>
      </div>
      <div class="game-card-body">
        ...
        <div class="game-card-tags">
          ${e.categories.map(o=>`<span class="game-tag">${o}</span>`).join("")}
        </div>
  ...
  `}function ue(e){e.querySelectorAll(".play-game-btn").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();const a=t.getAttribute("data-id");k(`#/game/${a}`)})}),e.querySelectorAll(".favorite-btn").forEach(t=>{t.addEventListener("click",async s=>{if(s.stopPropagation(),!l.user){p("אנא התחבר כדי לשמור משחקים מועדפים!","warning"),k("#/login");return}const a=t.getAttribute("data-id");let i=[...l.user.favorites||[]];i.includes(a)?(i=i.filter(n=>n!==a),t.classList.remove("active"),t.querySelector("i").className="far fa-heart",p("הוסר מהמועדפים","info")):(i.push(a),t.classList.add("active"),t.querySelector("i").className="fas fa-heart",p("נוסף למועדפים! ❤️","success")),l.user.favorites=i,await _(l.user.uid,{favorites:i}),mt()})})}function mt(){const e=document.getElementById("recent-played-section"),t=document.getElementById("recent-games-grid"),s=document.getElementById("favorites-section"),a=document.getElementById("favorite-games-grid");if(!l.user){e&&(e.style.display="none"),s&&(s.style.display="none");return}const i=l.user.recentlyPlayed||[];if(i.length>0&&t){const o=l.games.filter(r=>i.includes(r.id));o.length>0?(e.style.display="block",t.innerHTML=o.map(r=>ce(r)).join(""),ue(t)):e.style.display="none"}else e&&(e.style.display="none");const n=l.user.favorites||[];if(n.length>0&&a){const o=l.games.filter(r=>n.includes(r.id));o.length>0?(s.style.display="block",a.innerHTML=o.map(r=>ce(r)).join(""),ue(a)):s.style.display="none"}else s&&(s.style.display="none")}function Ct(){const e=document.getElementById("main-container");e.innerHTML=`
  ...
  `;let t=!1;const s=document.getElementById("login-form"),a=document.getElementById("toggle-auth-mode"),i=document.getElementById("auth-panel-title"),n=document.getElementById("auth-biometric-btn");a.addEventListener("click",o=>{o.preventDefault(),t=!t,t?(i.textContent="רישום חשבון DIGGY חדש",s.querySelector('button[type="submit"]').innerHTML='<i class="fas fa-user-plus"></i> צור חשבון',a.textContent="התחבר לחשבון קיים",n.style.display="none"):(i.textContent="כניסה למערכת DIGGY",s.querySelector('button[type="submit"]').innerHTML='<i class="fas fa-rocket"></i> התחבר',a.textContent="צור חשבון חדש",n.style.display="flex")}),s.addEventListener("submit",async o=>{o.preventDefault();const r=O(document.getElementById("auth-username").value),c=document.getElementById("auth-password").value,d=ge(r);if(!d.valid){p(d.errors[0],"danger");return}const g=me(c);if(!g.valid){p(g.errors[0],"danger");return}if(!t){const m=Be(r);if(!m.allowed){p(`יותר מדי ניסיונות כניסה. נסה שוב בעוד ${m.remainingTime} דקות.`,"danger");return}}y(!0);try{if(t){const m=await Pe(r,c);p("החשבון נוצר בהצלחה! ברוך הבא ל-DIGGY 🎉","success"),k("#/")}else{De(r);const m=await ye(r,c);Re(r);const x=fe(m);if(x.required&&!x.complete){y(!1),p("לפני הכניסה למערכת עליך להשלים הגדרות אבטחה: אימות דו-שלבי וכתובת תמיכה.","danger"),k("#/settings");return}if(m.twoFactorEnabled){y(!1),Mt(m);return}p("התחברת בהצלחה! 🎮","success"),k("#/")}}catch(m){p(m.message,"danger")}finally{y(!1)}}),n.addEventListener("click",()=>{qt()})}function Mt(e){const t=ne(e.uid),s=`
  ...
  `;J(()=>Promise.resolve().then(()=>$t),void 0).then(async a=>{const i=e.twoFactorEmail||e.email||"diggy-games@outlook.com";await a.sendEmailViaResend(i,"DIGGY - קוד אימות דו-שלבי",s);const n=document.getElementById("modal-overlay"),o=document.getElementById("modal-title"),r=document.getElementById("modal-body");o.textContent="אימות דו-שלבי (2FA)",r.innerHTML=`
  ...
  `,e.classList.add("active");let a=localStorage.getItem("diggy_bio_username"),i=localStorage.getItem("diggy_bio_uid");setTimeout(async()=>{const n=document.getElementById("bio-status"),o=document.getElementById("bio-widget");if(!a||!i){o.classList.remove("scanning"),o.style.color="var(--danger-color)",n.innerHTML="שגיאה: זיהוי ביומטרי לא מוגדר!",n.style.color="var(--danger-color)",setTimeout(()=>{e.classList.remove("active"),p("לא הוגדרה כניסה ביומטרית בחשבון זה! היכנס רגיל והפעל אותה בהגדרות.","warning")},1500);return}try{(await tt(a,i)).success&&(o.classList.remove("scanning"),o.style.color="#00ff66",n.innerHTML="סריקה הושלמה! מאושר",setTimeout(async()=>{e.classList.remove("active");const c=await ye(a,"auth_biometric_token");p(`ברוך שובך ביומטרי, ${a}!`,"success"),k("#/")},1e3))}catch(r){console.warn("WebAuthn verification failed:",r),o.classList.remove("scanning"),o.style.color="var(--danger-color)",n.innerHTML="סריקה נכשלה",n.style.color="var(--danger-color)",setTimeout(()=>{e.classList.remove("active"),p("שגיאה בכניסה ביומטרית: "+r.message,"danger")},1500)}},2e3)}async function xe(){const e=document.getElementById("main-container");if(!l.user||l.user.role!=="developer"&&l.user.role!=="admin"){e.innerHTML=`
  ...
  `;try{const t=await Ne(l.user.uid),s=document.getElementById("dev-games-list-body");t.length===0?s.innerHTML=`
        <tr>
      ...
      `:(s.innerHTML=t.map(a=>{let i="";a.status==="pending"?i='<span class="badge badge-pending">ממתין לאישור</span>':a.status==="approved"?i='<span class="badge badge-approved">אושר בהצלחה</span>':a.status==="rejected"?i='<span class="badge badge-rejected">נדחה</span>':a.status==="improvement"&&(i='<span class="badge badge-improvement">דרוש תיקון</span>');const n=a.status==="improvement"?`<button class="btn btn-secondary resubmit-btn" data-id="${a.id}" style="padding: 4px 10px; font-size: 11px;"><i class="fas fa-edit"></i> ערוך והגש שנית</button>`:a.status==="approved"?`<div style="display: flex; gap: 6px;">
        ...
        `}).join(""),s.querySelectorAll(".resubmit-btn").forEach(a=>{a.addEventListener("click",()=>{const i=a.closest("tr"),n=JSON.parse(i.getAttribute("data-raw"));_e(n)})}),s.querySelectorAll(".view-stats-btn").forEach(a=>{a.addEventListener("click",()=>{const i=a.closest("tr"),n=JSON.parse(i.getAttribute("data-raw"));zt(n)})}),s.querySelectorAll(".new-version-btn").forEach(a=>{a.addEventListener("click",()=>{const i=a.closest("tr"),n=JSON.parse(i.getAttribute("data-raw"));Ft(n)})}))}catch(t){p("שגיאה בטעינת משחקי מפתח: "+t.message,"danger")}document.getElementById("dev-submit-game-btn").addEventListener("click",()=>{_e()})}function _e(e=null){const t=document.getElementById("modal-overlay"),s=document.getElementById("modal-title"),a=document.getElementById("modal-body");s.textContent=e?`עריכת והגשת המשחק: ${e.name}`:"הגשת משחק חדש ל-DIGGY",a.innerHTML=`
  ...
  `,t.classList.add("active")}function Ft(e){const t=document.getElementById("modal-overlay"),s=document.getElementById("modal-title"),a=document.getElementById("modal-body");s.textContent=`הגשת גרסה חדשה: ${e.name}`,a.innerHTML=`
  ...
  `,t.classList.add("active"),document.getElementById("game-version-form").addEventListener("submit",async n=>{n.preventDefault();const o={version:document.getElementById("version-number").value.trim(),changelog:document.getElementById("version-changelog").value.trim(),gameUrl:document.getElementById("version-url").value.trim(),githubUrl:document.getElementById("version-github").value.trim(),developerUid:l.user.uid,developerName:l.user.username,name:e.name,logoUrl:e.logoUrl,description:e.description};y(!0);try{await nt(e.gameId||e.id,o),p("גרסת המשחק החדשה נשלחה לאישור המנהל! 🚀","success"),t.classList.remove("active"),xe()}catch(r){p(r.message,"danger")}finally{y(!1)}})}async function Q(){const e=document.getElementById("main-container");if(!l.user||l.user.role!=="admin"){e.innerHTML=`
      <div style="text-align: center; padding: 80px 0;">
  ...
  `,le(l.supportActiveThreadId),document.getElementById("admin-direct-upload-btn").addEventListener("click",()=>{Ot()});const t=document.getElementById("save-resend-config-btn");t&&t.addEventListener("click",()=>{const s=document.getElementById("emailjs-service-id").value,a=document.getElementById("emailjs-template-id").value,i=document.getElementById("emailjs-public-key").value,n=document.getElementById("emailjs-from-name").value,o=document.getElementById("support-admin-email").value,r=document.getElementById("site-support-email").value,c=document.getElementById("site-legal-email").value,d=document.getElementById("site-notification-email").value;it(s,a,i,n),Tt({supportEmail:r,legalEmail:c,notificationEmail:d}),o&&localStorage.setItem("diggy_support_admin_email",o),p("הגדרות EmailJS ואימיילים נשמרו.","success")});try{const s=await je(),a=document.getElementById("admin-dev-requests-body");s.length===0?a.innerHTML='<tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">אין פניות מפתחים פעילות.</td></tr>':(a.innerHTML=s.map(i=>{const n=i.status==="pending";let o="";i.status==="approved"?o='<span class="badge badge-approved">אושר</span>':i.status==="rejected"?o='<span class="badge badge-rejected">נדחה</span>':o='<span class="badge badge-pending">ממתין</span>';const r=n?`
        ...
        `}).join(""),a.querySelectorAll(".admin-approve-dev").forEach(i=>{i.addEventListener("click",()=>z(i.getAttribute("data-id"),"approved","dev"))}),a.querySelectorAll(".admin-reject-dev").forEach(i=>{i.addEventListener("click",()=>z(i.getAttribute("data-id"),"rejected","dev"))}))}catch(s){console.error("Error loading dev requests:",s)}try{const s=await Je(),a=document.getElementById("admin-game-requests-body");s.length===0?a.innerHTML='<tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">אין משחקים הממתינים לאישור.</td></tr>':(a.innerHTML=s.map(i=>{const n=i.status==="pending";let o="";i.status==="approved"?o='<span class="badge badge-approved">אושר</span>':i.status==="rejected"?o='<span class="badge badge-rejected">נדחה</span>':i.status==="improvement"&&(o='<span class="badge badge-improvement">הצעת שיפור</span>');const r=i.type==="version_update"?`<span class="badge badge-pending" style="background: rgba(112, 214, 255, 0.15); color: #70d6ff; border-color: rgba(112,214,255,0.3); margin-top: 4px; display: inline-block;">עדכון גרסה (${i.version})</span>`:"",c=n?`
          ...
          `:`<div style="display: flex; flex-direction: column; gap: 4px;">${o}<span style="color: var(--text-muted); font-size: 11px;">${i.adminSuggestions||""}</span></div>`;return`
          <tr>
        ...
        `}).join(""),a.querySelectorAll(".admin-approve-game").forEach(i=>{i.addEventListener("click",()=>z(i.getAttribute("data-id"),"approved","game"))}),a.querySelectorAll(".admin-improve-game").forEach(i=>{i.addEventListener("click",()=>z(i.getAttribute("data-id"),"improvement","game"))}),a.querySelectorAll(".admin-reject-game").forEach(i=>{i.addEventListener("click",()=>z(i.getAttribute("data-id"),"rejected","game"))}))}catch(s){console.error("Error loading games queue:",s)}try{const s=await ze(),a=document.getElementById("admin-users-list-body");s.length===0?a.innerHTML='<tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">לא נמצאו חשבונות רשומים.</td></tr>':(a.innerHTML=s.map(i=>{const n=i.createdAt?new Date(i.createdAt).toLocaleDateString():"לא ידוע",o=i.twoFactorEnabled?'<span style="color: var(--accent-color); font-size: 11px;"><i class="fas fa-check-circle"></i> פעיל</span>':'<span style="color: var(--text-dark); font-size: 11px;">כבוי</span>',r=i.biometricsEnabled?'<span style="color: var(--accent-color); font-size: 11px;"><i class="fas fa-check-circle"></i> פעיל</span>':'<span style="color: var(--text-dark); font-size: 11px;">כבוי</span>';return`
          <tr>
        ...
        `}).join(""),a.querySelectorAll(".admin-role-select").forEach(i=>{i.addEventListener("change",async()=>{const n=i.getAttribute("data-uid"),o=i.value;y(!0);try{await Fe(n,o),p(`דרגת המשתמש עודכנה ל-${o.toUpperCase()} בהצלחה!`,"success"),l.user&&l.user.uid===n&&(l.user.role=o,Z()),Q()}catch(r){p("עדכון הדרגה נכשל: "+r.message,"danger")}finally{y(!1)}})}))}catch(s){console.error("Error loading users list:",s)}}function z(e,t,s){const a=document.getElementById("modal-overlay"),i=document.getElementById("modal-title"),n=document.getElementById("modal-body");i.textContent="הזנת הסבר מנהל מערכת (Admin Action)";let o="רשום סיבה או הצעות לשיפור שיועברו למשתמש:";t==="approved"?o="הערות אישור (יופיעו במייל):":t==="rejected"?o="סיבת סירוב (יופיע במייל - המשתמש לא יוכל להגיש שוב):":t==="improvement"&&(o="פרט את ההצעות לשיפור ושינויים שנדרשים מהמפתח:"),n.innerHTML=`
  ...
  `,a.classList.add("active"),document.getElementById("admin-reason-form").addEventListener("submit",async r=>{r.preventDefault();const c=document.getElementById("admin-notes").value.trim();y(!0);try{s==="dev"?(await He(e,t,c),p("בקשת המפתח עודכנה והמייל נשלח בהצלחה!","success")):s==="game"&&(await Ke(e,t,c),p("בקשת המשחק עודכנה והמייל נשלח בהצלחה!","success")),a.classList.remove("active"),await be(),Q()}catch(d){p(d.message,"danger")}finally{y(!1)}})}function Ot(){const e=document.getElementById("modal-overlay"),t=document.getElementById("modal-title"),s=document.getElementById("modal-body");t.textContent="העלאה ישירה של משחק (Admin Bypass)",s.innerHTML=`
  ...
  `,e.classList.add("active");const a=document.getElementById("admin-direct-upload-form");a.addEventListener("submit",async i=>{i.preventDefault();const n=a.querySelectorAll('input[name="direct-cats"]:checked');if(n.length===0){p("בחר לפחות קטגוריה אחת!","warning");return}const o=Array.from(n).map(c=>c.value),r={name:document.getElementById("direct-name").value,description:document.getElementById("direct-desc").value,logoUrl:document.getElementById("direct-logo").value,githubUrl:document.getElementById("direct-github").value,gameUrl:document.getElementById("direct-url").value,howToPlay:document.getElementById("direct-how").value,targetAudience:document.getElementById("direct-audience").value,categories:o,developerUid:l.user.uid,developerName:`${l.user.username} (ADMIN)`};y(!0);try{await We(r),p("המשחק פורסם בהצלחה באתר ללא צורך באישור! 🎉","success"),e.classList.remove("active"),await be(),Q()}catch(c){p(c.message,"danger")}finally{y(!1)}})}async function gt(e){const t=document.getElementById("main-container"),s=l.games.find(a=>a.id===e);if(!s){t.innerHTML='<div style="text-align: center; padding: 80px 0;"><h2>משחק לא נמצא!</h2></div>';return}if(l.currentGame=s,l.user){let a=[...l.user.recentlyPlayed||[]];a=a.filter(i=>i!==e),a.unshift(e),a=a.slice(0,5),l.user.recentlyPlayed=a,await _(l.user.uid,{recentlyPlayed:a})}t.innerHTML=`
    ...
    <div class="game-play-area">
      ...
      <div class="game-screen-panel">
        <div class="game-screen-header">
          <h2 style="font-size: 20px; color: var(--accent-color);">${s.name}</h2>
        ...
        <div class="game-canvas-container">
          <div class="game-menu-overlay" id="game-menu-overlay">
            <h3 class="game-menu-title">${s.name}</h3>
            <p style="color: var(--text-muted); font-size: 14px; max-width: 380px; text-align: center; line-height: 1.5;">${s.howToPlay}</p>
      ...
      <div class="game-sidebar-panel">
        <div style="display: flex; justify-content: center;">
          <img src="${s.logoUrl}" onerror="this.src='https://placehold.co/120x120/12161e/00ff66?text=DIGGY'" style="width: 100px; height: 100px; border-radius: 12px; object-fit: cover; border: 2px solid var(--accent-color); box-shadow: var(--border-glow);">
        </div>
        
        <div class="game-meta-item">
          <span class="game-meta-label">מפתח</span>
          <span class="game-meta-val" style="font-weight: bold; color: var(--accent-color);">${s.developerName}</span>
        </div>

        <div class="game-meta-item">
          <span class="game-meta-label">קטגוריות</span>
          <div style="display: flex; gap: 5px;">
            ${s.categories.map(a=>`<span class="game-tag" style="background: var(--accent-dim); color: var(--accent-color);">${a}</span>`).join("")}
          </div>
        </div>

        <div class="game-meta-item">
          <span class="game-meta-label">קהל יעד</span>
          <span class="game-meta-val">${s.targetAudience}</span>
        </div>
        ...
        <div class="game-meta-item">
          ...
          <p style="font-size: 13px; line-height: 1.5; color: var(--text-muted);">${s.description}</p>
        </div>
      </div>
    </div>
  `,document.getElementById("start-game-btn").addEventListener("click",()=>{if(document.getElementById("game-menu-overlay").style.display="none",st(s.id).catch(a=>console.warn("Failed to record play stat:",a)),s.gameUrl){const a=document.getElementById("retro-game-iframe");a.src=s.gameUrl,a.style.display="block",l.gameInstance={stop:()=>{a.src="",a.style.display="none"}}}else{const a=document.getElementById("retro-game-canvas");a.style.display="block",s.id==="preset_snake"?l.gameInstance=Te(a):s.id==="preset_bricks"?l.gameInstance=jt(a):s.id==="preset_evader"?l.gameInstance=Ht(a):l.gameInstance=Te(a)}}),ut(e)}function Te(e){const t=e.getContext("2d"),s=document.getElementById("game-score-display");let a=0,i=20,n=0,o={x:160,y:160,dx:i,dy:0,cells:[],maxCells:4},r={x:320,y:320},c=null,d=!0;function g(b,R){return Math.floor(Math.random()*(R-b))+b}function m(){d&&(c=requestAnimationFrame(m),!(++n<6)&&(n=0,t.clearRect(0,0,e.width,e.height),o.x+=o.dx,o.y+=o.dy,o.x<0?o.x=e.width-i:o.x>=e.width&&(o.x=0),o.y<0?o.y=e.height-i:o.y>=e.height&&(o.y=0),o.cells.unshift({x:o.x,y:o.y}),o.cells.length>o.maxCells&&o.cells.pop(),t.fillStyle="#ff3366",t.shadowBlur=15,t.shadowColor="#ff3366",t.beginPath(),t.arc(r.x+i/2,r.y+i/2,i/2-2,0,2*Math.PI),t.fill(),t.fillStyle=l.theme,t.shadowBlur=15,t.shadowColor=l.theme,o.cells.forEach(function(b,R){t.fillRect(b.x,b.y,i-1,i-1),b.x===r.x&&b.y===r.y&&(o.maxCells++,a+=10,s.textContent=`ניקוד: ${a}`,r.x=g(0,e.width/i)*i,r.y=g(0,e.height/i)*i);for(let f=R+1;f<o.cells.length;f++)b.x===o.cells[f].x&&b.y===o.cells[f].y&&A()})))}function x(b){b.key==="ArrowLeft"&&o.dx===0?(o.dx=-i,o.dy=0):b.key==="ArrowUp"&&o.dy===0?(o.dy=-i,o.dx=0):b.key==="ArrowRight"&&o.dx===0?(o.dx=i,o.dy=0):b.key==="ArrowDown"&&o.dy===0&&(o.dy=i,o.dx=0)}document.addEventListener("keydown",x),c=requestAnimationFrame(m);function A(){d=!1,t.shadowBlur=0,t.fillStyle="rgba(0, 0, 0, 0.8)",t.fillRect(0,0,e.width,e.height),t.fillStyle="#ff3366",t.font="24px Orbitron",t.textAlign="center",t.fillText("GAME OVER",e.width/2,e.height/2-20),t.fillStyle="#ffffff",t.font="14px Outfit",t.fillText(`ניקוד סופי: ${a}`,e.width/2,e.height/2+10),t.fillText("לחץ שוב על כפתור התחל כדי לנסות שנית",e.width/2,e.height/2+40)}return{stop:()=>{d=!1,cancelAnimationFrame(c),document.removeEventListener("keydown",x)}}}function jt(e){const t=e.getContext("2d"),s=document.getElementById("game-score-display");let a=0,i=!0,n=null,o={x:e.width/2,y:e.height-30,dx:3,dy:-3,radius:8},r={x:e.width/2-50,y:e.height-20,width:100,height:10,speed:7},c=!1,d=!1,g=4,m=6,x=75,A=20,b=15,R=40,f=40,$=[];const G=["#ff3366","#00ff66","#0096ff","#ffcc00"];for(let h=0;h<m;h++){$[h]=[];for(let S=0;S<g;S++)$[h][S]={x:0,y:0,status:1,color:G[S%G.length]}}function Y(h){h.key==="Right"||h.key==="ArrowRight"?d=!0:(h.key==="Left"||h.key==="ArrowLeft")&&(c=!0)}function Ee(h){h.key==="Right"||h.key==="ArrowRight"?d=!1:(h.key==="Left"||h.key==="ArrowLeft")&&(c=!1)}function Ie(h){let S=h.clientX-e.getBoundingClientRect().left;S>0&&S<e.width&&(r.x=S-r.width/2)}document.addEventListener("keydown",Y),document.addEventListener("keyup",Ee),document.addEventListener("mousemove",Ie);function yt(){for(let h=0;h<m;h++)for(let S=0;S<g;S++){let B=$[h][S];B.status===1&&o.x>B.x&&o.x<B.x+x&&o.y>B.y&&o.y<B.y+A&&(o.dy=-o.dy,B.status=0,a+=15,s.textContent=`ניקוד: ${a}`,a===g*m*15&&ht())}}function Se(){if(i){t.clearRect(0,0,e.width,e.height);for(let h=0;h<m;h++)for(let S=0;S<g;S++)if($[h][S].status===1){let B=h*(x+b)+f,Ae=S*(A+b)+R;$[h][S].x=B,$[h][S].y=Ae,t.fillStyle=$[h][S].color,t.shadowBlur=10,t.shadowColor=$[h][S].color,t.fillRect(B,Ae,x,A)}if(t.beginPath(),t.arc(o.x,o.y,o.radius,0,Math.PI*2),t.fillStyle="#ffffff",t.shadowBlur=12,t.shadowColor="#ffffff",t.fill(),t.closePath(),t.fillStyle=l.theme,t.shadowBlur=15,t.shadowColor=l.theme,t.fillRect(r.x,r.y,r.width,r.height),yt(),(o.x+o.dx>e.width-o.radius||o.x+o.dx<o.radius)&&(o.dx=-o.dx),o.y+o.dy<o.radius)o.dy=-o.dy;else if(o.y+o.dy>e.height-o.radius)if(o.x>r.x&&o.x<r.x+r.width)o.dy=-o.dy;else{vt();return}d&&r.x<e.width-r.width?r.x+=r.speed:c&&r.x>0&&(r.x-=r.speed),o.x+=o.dx,o.y+=o.dy,n=requestAnimationFrame(Se)}}n=requestAnimationFrame(Se);function vt(){i=!1,t.shadowBlur=0,t.fillStyle="rgba(0,0,0,0.85)",t.fillRect(0,0,e.width,e.height),t.fillStyle="#ff3366",t.font="24px Orbitron",t.textAlign="center",t.fillText("GAME OVER",e.width/2,e.height/2-20),t.fillStyle="#ffffff",t.font="14px Outfit",t.fillText(`ניקוד סופי: ${a}`,e.width/2,e.height/2+10),t.fillText("נסה שנית!",e.width/2,e.height/2+40)}function ht(){i=!1,t.shadowBlur=0,t.fillStyle="rgba(0,0,0,0.85)",t.fillRect(0,0,e.width,e.height),t.fillStyle="#00ff66",t.font="24px Orbitron",t.textAlign="center",t.fillText("YOU WIN!",e.width/2,e.height/2-20),t.fillStyle="#ffffff",t.fillText(`ניקוד סופי: ${a}`,e.width/2,e.height/2+10)}return{stop:()=>{i=!1,cancelAnimationFrame(n),document.removeEventListener("keydown",Y),document.removeEventListener("keyup",Ee),document.removeEventListener("mousemove",Ie)}}}function Ht(e){const t=e.getContext("2d"),s=document.getElementById("game-score-display");let a=0,i=!0,n=null,o={x:e.width/2-20,y:e.height-40,width:40,height:30,speed:6},r=[],c=[],d={},g=0;function m(f){d[f.key]=!0}function x(f){d[f.key]=!1}document.addEventListener("keydown",m),document.addEventListener("keyup",x);function A(){c.push({x:Math.random()*(e.width-30),y:-30,width:30,height:30,speed:1.5+Math.random()*3,color:"#ffaa00"})}function b(){i&&((d.ArrowLeft||d.a)&&(o.x=Math.max(0,o.x-o.speed)),(d.ArrowRight||d.d)&&(o.x=Math.min(e.width-o.width,o.x+o.speed)),(d[" "]||d.Spacebar)&&(!o.lastFired||Date.now()-o.lastFired>300)&&(r.push({x:o.x+o.width/2-2,y:o.y,width:4,height:12,speed:7}),o.lastFired=Date.now()),t.clearRect(0,0,e.width,e.height),t.fillStyle=l.theme,t.shadowBlur=15,t.shadowColor=l.theme,t.beginPath(),t.moveTo(o.x+o.width/2,o.y),t.lineTo(o.x,o.y+o.height),t.lineTo(o.x+o.width,o.y+o.height),t.closePath(),t.fill(),g++,g>40&&(A(),g=0),t.fillStyle="#00ffff",t.shadowColor="#00ffff",r.forEach((f,$)=>{f.y-=f.speed,t.fillRect(f.x,f.y,f.width,f.height),f.y<0&&r.splice($,1)}),c.forEach((f,$)=>{if(f.y+=f.speed,t.fillStyle=f.color,t.shadowColor=f.color,t.fillRect(f.x,f.y,f.width,f.height),f.x<o.x+o.width&&f.x+f.width>o.x&&f.y<o.y+o.height&&f.y+f.height>o.y){R();return}r.forEach((G,Y)=>{G.x<f.x+f.width&&G.x+G.width>f.x&&G.y<f.y+f.height&&G.y+G.height>f.y&&(c.splice($,1),r.splice(Y,1),a+=20,s.textContent=`ניקוד: ${a}`)}),f.y>e.height&&c.splice($,1)}),n=requestAnimationFrame(b))}n=requestAnimationFrame(b);function R(){i=!1,t.shadowBlur=0,t.fillStyle="rgba(0,0,0,0.85)",t.fillRect(0,0,e.width,e.height),t.fillStyle="#ff3366",t.font="24px Orbitron",t.textAlign="center",t.fillText("SPACE SHUTTLE CRASHED",e.width/2,e.height/2-20),t.fillStyle="#ffffff",t.font="14px Outfit",t.fillText(`ניקוד סופי: ${a}`,e.width/2,e.height/2+10),t.fillText("נסה שנית!",e.width/2,e.height/2+40)}return{stop:()=>{i=!1,cancelAnimationFrame(n),document.removeEventListener("keydown",m),document.removeEventListener("keyup",x)}}}function ft(){const e=document.getElementById("main-container");if(!l.user){e.innerHTML=`
      <div style="text-align: center; padding: 80px 0;">
        <h2>גישה מוגבלת</h2>
    ...
    <div style="display: flex; gap: 30px; flex-wrap: wrap;">
      ${(()=>{const o=fe(l.user);return o.required?`
            <div style="width: 100%; padding: 14px 16px; border-radius: 12px; background: ${o.complete?"rgba(0,255,102,0.08)":"rgba(255,170,0,0.12)"}; border: 1px solid ${o.complete?"rgba(0,255,102,0.25)":"rgba(255,170,0,0.25)"}; margin-bottom: 20px;">
              <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                <i class="fas ${o.complete?"fa-shield-alt":"fa-exclamation-triangle"}" style="color: ${o.complete?"var(--accent-color)":"#ffaa00"};"></i>
                <strong style="font-size: 14px;">${o.complete?"החשבון מוכן לשימוש":"נדרשות הגדרות אבטחה לפיתוח/ניהול"}</strong>
              </div>
              <div style="font-size: 13px; color: var(--text-muted); line-height: 1.6;">
                ${o.complete?"כל הדרישות הושלמו ותוכל להמשיך לעבוד עם החשבון.":`לפני המשך העבודה יש להשלים:${o.missingItems.includes("twoFactor")?"<br>• הפעלת אימות דו-שלבי":""}${o.missingItems.includes("supportEmail")?"<br>• הזנת כתובת תמיכה פנימית":""}`}
              </div>
            </div>
          `:""})()}
    ...
    `:""}
  `,document.getElementById("save-profile-btn").addEventListener("click",async()=>{const o=O(document.getElementById("settings-username").value.trim()),r=ge(o);if(!r.valid){p(r.errors[0],"danger");return}y(!0);try{await _(l.user.uid,{username:o}),l.user.username=o,Z(),p("שם המשתמש עודכן בהצלחה!","success")}catch(c){p(c.message,"danger")}finally{y(!1)}}),document.getElementById("change-pass-btn").addEventListener("click",async()=>{const o=document.getElementById("settings-new-password").value,r=me(o);if(!r.valid){p(r.errors[0],"danger");return}y(!0);try{await qe(o),p("הסיסמה שונתה בהצלחה!","success"),document.getElementById("settings-new-password").value=""}catch(c){p(c.message,"danger")}finally{y(!1)}});const t=document.getElementById("settings-2fa-toggle"),s=document.getElementById("settings-2fa-email-group");t.addEventListener("change",async()=>{const o=t.checked;if(s.style.display=o?"block":"none",!o){if(W(l.user.role)){t.checked=!0,s.style.display="block",p("למשתמשי מפתח/Admin חובה להפעיל אימות דו-שלבי.","danger");return}y(!0),await _(l.user.uid,{twoFactorEnabled:!1}),l.user.twoFactorEnabled=!1,p("אימות דו-שלבי בוטל.","info"),y(!1)}});const a=document.getElementById("settings-2fa-email");a.addEventListener("change",async()=>{const o=O(a.value.trim());if(!o){p("יש להזין כתובת אימייל לשליחת הקוד","danger");return}const r=V(o);if(!r.valid){p(r.error,"danger");return}y(!0),await _(l.user.uid,{twoFactorEnabled:!0,twoFactorEmail:o}),l.user.twoFactorEnabled=!0,l.user.twoFactorEmail=o,p("כתובת אימות דו-שלבי עודכנה!","success"),y(!1)});const i=document.getElementById("settings-support-email");i&&i.addEventListener("change",async()=>{const o=O(i.value.trim());if(!o){p("יש להזין כתובת תמיכה פנימית","danger");return}const r=V(o);if(!r.valid){p(r.error,"danger");return}y(!0),await _(l.user.uid,{supportEmail:o}),l.user.supportEmail=o,p("כתובת התמיכה נשמרה בהצלחה!","success"),y(!1)}),document.querySelectorAll(".color-picker-btn").forEach(o=>{o.addEventListener("click",async()=>{document.querySelectorAll(".color-picker-btn").forEach(c=>c.classList.remove("active")),o.classList.add("active");const r=o.getAttribute("data-color");y(!0),await _(l.user.uid,{customTheme:r}),l.user.customTheme=r,pe(r),y(!1),p("ערכת העיצוב הניאונית עודכנה!","success")})}),document.getElementById("register-biometric-btn").addEventListener("click",async()=>{y(!0);try{const o=await et(l.user.username,l.user.uid);o.success&&(localStorage.setItem("diggy_bio_username",l.user.username),localStorage.setItem("diggy_bio_uid",l.user.uid),await _(l.user.uid,{biometricsEnabled:!0,biometricsCredentialId:o.credentialId.join(",")}),l.user.biometricsEnabled=!0,document.getElementById("bio-setup-status").textContent="מופעל",document.getElementById("bio-setup-status").style.color="var(--accent-color)",p("זיהוי ביומטרי הופעל בהצלחה עבור מכשיר זה! 🔒","success"))}catch(o){p("שגיאה ברישום ביומטרי: "+o.message,"danger")}finally{y(!1)}});const n=document.getElementById("dev-application-form");n&&n.addEventListener("submit",async o=>{o.preventDefault();const r=document.getElementById("dev-app-reason").value,c=document.getElementById("dev-app-email").value,d=document.getElementById("dev-app-pass").value;if(d.length<6||d.length>12){p("הזן סיסמת אימות תקינה (6-12 תווים)!","danger");return}y(!0);try{await Oe(l.user.uid,l.user.username,r,c),p("בקשת המפתח נשלחה בהצלחה! מייל עיצוב יישלח אלייך עם החלטת ה-Admin. 📬","success"),ft()}catch(g){p(g.message,"danger")}finally{y(!1)}})}function pe(e){l.theme=e;const t=document.documentElement;t.style.setProperty("--accent-color",e),t.style.setProperty("--accent-glow",`${e}66`),t.style.setProperty("--accent-dim",`${e}1a`),t.style.setProperty("--border-color",`${e}26`)}function Z(){const e=document.getElementById("sidebar-user-badge-wrap");if(e)if(l.user){const t=l.user.username.charAt(0).toUpperCase();e.innerHTML=`
    ...
    `}};async function Yt(){q();const e=document.getElementById("main-container"),t=Object.entries(we);e.innerHTML=`
    <div class="top-header">
      <div class="page-title-wrap">
        <h1>מאמרים וחדשות</h1>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; margin-top: 20px;">
      ${t.map(([s,a])=>`
        <div class="article-card" data-slug="${s}">
  ...
  `,e.querySelectorAll(".article-card").forEach(s=>{s.addEventListener("click",()=>{k(`#/articles/${s.getAttribute("data-slug")}`)})})}async function Nt(e){q();const t=document.getElementById("main-container"),s=we[e];if(!s){t.innerHTML=`
      <div style="text-align: center; padding: 80px 0;">
        <h2>מאמר לא נמצא</h2>
    ...
    <div class="legal-page-content">
      <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 15px;"><i class="fas fa-calendar-alt"></i> ${s.date}</div>
      <h2 class="doc-article-title"><i class="fas ${s.icon}"></i> ${s.title}</h2>
      <div class="doc-section">${s.content}</div>
    </div>
  `}async function Jt(){q();const e=document.getElementById("main-container"),t=l.games.map(a=>`<li><a href="#/game/${a.id}">${a.name}</a></li>`).join(""),s=Object.entries(we).map(([a,i])=>`<li><a href="#/articles/${a}">${i.title}</a></li>`).join("");e.innerHTML=`
    <div class="top-header">
      <div class="page-title-wrap">
        <h1>מפת האתר</h1>
  ...
  `}async function Wt(){q();const e=document.getElementById("main-container");e.innerHTML=`
    <div class="top-header">
      <div class="page-title-wrap">
        <h1>צור קשר</h1>
  ...
  `;const t=document.getElementById("support-request-form");t&&t.addEventListener("submit",async s=>{s.preventDefault();const a=document.getElementById("support-name").value.trim(),i=document.getElementById("support-email").value.trim(),n=document.getElementById("support-subject").value.trim(),o=document.getElementById("support-message").value.trim();if(!a||!i||!n||!o){p("אנא מלא את כל השדות.","warning");return}y(!0);try{const r=Gt({name:a,email:i,subject:n,message:o}),c=P().supportEmail||localStorage.getItem("diggy_support_admin_email")||"diggy-games@outlook.com",d=`
          <div style="font-family: sans-serif; background: #07080a; color: white; padding: 24px; border-radius: 12px; border: 1px solid #00ff66;">
            <h2 style="color: #00ff66;">פנייה חדשה לתמיכה - DIGGY</h2>
            <p><strong>שם:</strong> ${a}</p>
            <p><strong>אימייל:</strong> ${i}</p>
            <p><strong>נושא:</strong> ${n}</p>
            <p><strong>הודעה:</strong> ${o}</p>
          </div>
        `,g=`
          <div style="font-family: sans-serif; background: #07080a; color: white; padding: 24px; border-radius: 12px; border: 1px solid #00ff66;">
        ...
        `;await H(c,`DIGGY Support: ${n}`,d),await H(i,"DIGGY - קיבלנו את הפנייה שלך",g),p("הפנייה נשלחה בהצלחה! אנחנו נענה בקרוב.","success"),t.reset()}catch(r){p(r.message||"שגיאה בשליחת הפנייה","danger")}finally{y(!1)}})}async function Zt(){const e=document.getElementById("main-container");if(!l.user||l.user.role!=="developer"&&l.user.role!=="admin"){e.innerHTML=`
      <div style="text-align: center; padding: 80px 0;">
        <i class="fas fa-lock" style="font-size: 64px; color: var(--danger-color); margin-bottom: 20px;"></i>
        <h2>גישה חסומה!</h2>
...
&lt;/html&gt;</div>
    ...
    `};function s(i){const n=document.getElementById("doc-content-area");n.innerHTML=t[i]||""}s("getting-started");const a=document.querySelectorAll(".doc-tab-btn");a.forEach(i=>{i.addEventListener("click",()=>{a.forEach(o=>o.classList.remove("active-doc-tab")),i.classList.add("active-doc-tab");const n=i.getAttribute("data-doc");s(n)})})}
